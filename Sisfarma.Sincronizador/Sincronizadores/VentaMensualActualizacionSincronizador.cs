...
namespace Sisfarma.Sincronizador.Sincronizadores
{
    public class VentaMensualActualizacionSincronizador : TaskSincronizador
    {
        ...
        private const string FIELD_REVISAR_VENTA_MES_DESDE = FieldsConfiguracion.FIELD_REVISAR_VENTA_MES_DESDE;
        private const string FIELD_FECHA_PUNTOS = FieldsConfiguracion.FIELD_FECHA_PUNTOS;
        private const string FIELD_CARGAR_PUNTOS = FieldsConfiguracion.FIELD_FECHA_PUNTOS;
        ...
        private void ProcessVentaMensual()
        {
            ...
            foreach (var venta in ventas)
            {
                ...
                {                                                
                    ...
                    {
                        var cargarPuntos = _fisiotes.Configuraciones.GetByCampo(FIELD_CARGAR_PUNTOS);
                ...
                if (newInsert && 
                    dni != 0 && fechaDePuntos.ToLower() != "no" &&
                    !string.IsNullOrWhiteSpace(fechaDePuntos) &&
                    fechaDeVenta >= fechaDePuntos.ToDateTimeOrDefault("yyyyMMdd"))
                {
        ...
        {
            var mesConfiguracion = _fisiotes.Configuraciones.GetByCampo(FIELD_REVISAR_VENTA_MES_DESDE)
                .ToIntegerOrDefault();
        ...
        private InsertPuntuacion GenerarPuntoPendienteCargadoPorDefault(int dniDelCliente, Venta venta, LineaVenta detalleDeVenta, Articulo articulo, string vendedor)
        {
            ...
            if (articulo != null)
            {
                familia = _farmatic.Familias.GetById(articulo.XFam_IdFamilia)?.Descripcion.Strip();
